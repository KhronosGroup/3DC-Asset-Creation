...
<header> 
    <h1>Real-time Asset Creation Guidelines (Abridged Version)</h1> 
</header> 
...
The contents of this document are under NDA and should not be shared or discussed outside of the Khronos group or 3D Commerce Advisory Forum mailing list.
...
[<ins>Link to full version</ins>][link-id-001]
...
* Use valid character sets for asset and file naming: `a-z`, `_`, `-`, `0–9`. 
    ...
    * Use `_` (underscore ) or `-` (hyphen) to separate different words.
...
[<ins>Link to full version</ins>][link-id-002]
...
* Bevel all product edges to smooth out edge transitions. There are no perfectly pointed edges in everyday life and your model should reflect this.
* Use watertight meshes.
* For opaque and transparent components, use separate meshes and materials
* Clear transform and construction history data to avoid future discrepancies

Link [<ins>here</ins>](https://www.google.com) for more details about geometry.
...
Link [<ins>here</ins>](https://www.google.com) for more details about UV coordinates.
...
* Transparent parts should always use a separate material from opaque parts.
...
Link [<ins>here</ins>](https://www.google.com) for more details about materials.
...
* Use sRGB color space for albedo and emissive and linear color space for all other textures.
* Texture types:
    * Albedo. If a transparency texture is needed, use the alpha channel of albedo.
    * ORM. Single bitmap with ambient occlusion in red channel, roughness in green channel, metalness in blue channel.
...
Link [<ins>here</ins>](https://www.google.com) for more details about textures.
...
* **Image-based Lighting (IBL)** are a panoramic environment image, used for both specular reflections (glossy surfaces) and soft diffuse lighting (rough surfaces). IBLs can be created from panoramic high-dynamic range photography or rendered from a computer graphics scene. Click here to link to the standard [<ins>Khronos sample GLTF viewer</ins>][link-id-gltfviewer]. Link [<ins>here</ins>](https://www.google.com) with common IBLs.
...
Link [<ins>here</ins>](https://www.google.com) for more details about lighting.
...
In order to best make certain decisions when authoring 3D Models, one must have an idea about the intended hardware and software where assets will ultimately be rendered for consumer visualization. Visual fidelity must be balanced with real-time performance and tested on the desired rendering platform. The following list specifies general guidelines for common publishing targets, aiming at the lowest common denominators.

* Web for Desktop and Mobile (for a single item)
    * File Size = Ideally less than 5MB **
    * Triangle Count = 110k or less
    * Textures = 2k albedo/transparency, 1k ORM/normal/emissive
    * Draw calls = fewer the better (e.g. consolidate meshes, fewer use of total materials to increase asset performance)

\[Note\]: File size recommendations will change as new texture compression formats such as KTX2 and Draco, with higher compression, become the new mainstream standard. To learn more about [<ins>KTX2</ins>][link-id-ktx2] and [<ins>Draco</ins>][link-id-draco].

Link [<ins>here</ins>](https://www.google.com) for more details about publishing targets.
...
[link-id-001]:/detail-version/FileFormatsAndAssetStructure.md#file-formats-and-asset-structure
[link-id-002]:/detail-version/CoordinateSystemandScaleUnit.md
=======
[link-id-001]:./detail-version/FileFormatsAndAssetStructure.md#file-formats-and-asset-structure
[link-id-002]:./detail-version/CoordinateSystemandScaleUnit.md
>>>>>>> bc8a0628c2ce1e82440c8c42bc3d3b3fcf4a09dc
